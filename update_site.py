...
def run_pipeline():
    ...
    if not scorecard.empty:
        ...
        # Standardize index column name
        idx_col = next((c for c in scorecard.columns if 'index' in c.lower() or 'ranking' in c.lower() or 'hi' in c.lower()), 'Sterling_Index')
        scorecard = scorecard.rename(columns={idx_col: 'Sterling_Index'})
    ...
    # A. Money Lists
    # 2025
    df_2025 = base[base['Tournament_Year'] == 2025].groupby('Player').agg({
    ...
    df_2025.columns = ['Name', 'Best_Ball_Earnings', 'Net_Skins_Earnings', 'Gross_Skins_Earnings', 'Net_Earnings', 'Total Earnings']
    inject_to_html("MoneyList2025.html", "csvData", df_2025.to_csv(index=False))

    # 2026 (Includes Quota)
    df_2026 = base[base['Tournament_Year'] == 2026].groupby('Player').agg({
        'BB_Earn': 'sum', 'Quota_Earn': 'sum', 'Nskins_earnings': 'sum', 'Gskins_earnings': 'sum', 'net_medal_earnings': 'sum', 'Total_Earnings': 'sum'
    }).reset_index().sort_values('Total_Earnings', ascending=False)
    df_2026.columns = ['Name', 'Best_Ball_Earnings', 'Quota_Earnings', 'Net_Skins_Earnings', 'Gross_Skins_Earnings', 'Net_Earnings', 'Total Earnings']
    inject_to_html("MoneyList2026.html", "csvData", df_2026.to_csv(index=False))
    ...
    p_stats['Net_to_Par'] = (p_stats['net_score'] - PAR_BASELINE).round(2)
    ...
    if not scorecard.empty:
        ...
        hole_avg['Par'] = 4
        hole_avg['Sterling_Index'] = hole_avg['hole']

    # Fix NaN for Relative to Par
    hole_avg['rel'] = hole_avg['score'] - hole_avg['Par']
    inject_to_html("AverageScore.html", "holeData", hole_avg[['hole', 'score', 'Par']].to_dict('records'), is_json=True)
    
    # Fix Hole Index Scatter Plot
    # Order Tournament Rank by (Average Score - Par)
    hole_avg['SG-SG_Rank'] = hole_avg['rel'].rank(ascending=False).astype(int)
    index_data = []
    for _, row in hole_avg.iterrows():
        index_data.append({
            "hole": int(row['hole']),
            "x": int(row['Sterling_Index']),
            "y": int(row['SG-SG_Rank'])
        })
...
def sync():
    ...
    subprocess.run(["git", "commit", "-m", "Fixed NaN in hole averages and repopulated money lists"], cwd=BASE_PATH)
    subprocess.run(["git", "push"], cwd=BASE_PATH)
    print("🎉 All Dashboards Corrected and Live!")
