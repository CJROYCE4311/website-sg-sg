...
def update_index_html(writeup_html):
    ...
    # Replace content. 
    # We target the prose div, capture everything up to the closing div of the prose block.
    # We rely on the fact that the content ends with the Run Date paragraph and then the closing </div> of the prose block.
    # The prose block is followed by the closing div of the p-8 container.
    # So we look for: <div class="prose ..."> ... Run Date ... </p></div>
    
    pattern = r'(<div class="prose prose-green max-w-none text-gray-600 space-y-4">).*?(Run Date: \d{4}-\d{2}-\d{2}</p>\s*</div>)'
    
    # Check if the specific Run Date pattern exists (it should if the file was generated by this script)
    # If not, fallback to the previous logic but maybe clearer?
    # Actually, the previous logic failed because it stopped early. 
    # Let's try to match until the closing div that follows the date.
    
    if re.search(pattern, html, flags=re.DOTALL):
        html = re.sub(pattern, r'\1' + writeup_html + '</div>', html, flags=re.DOTALL)
    else:
         # Fallback for clean file or initial run where date might differ
         # Match until the first </div> that follows "Run Date"
         pass
         # Actually, better regex:
         # Match <div class="prose..."> [content] </div>
         # But [content] contains nested divs.
         # So we simply replace the whole logical block if we can find the "Latest Results" header.
         
         # Let's stick to the specific Date pattern anchor as it's generated by this script.
         # If the date format changes, this breaks, so we keep the format consistent.
         # The generate_writeup_html function outputs: ...Run Date: YYYY-MM-DD</p>
         # So we construct the replacement to include the closing </div> of the prose div which generate_writeup_html DOES NOT output.
         # Wait, generate_writeup_html returns just the INNER content?
         # No, it returns divs and ps. It does NOT return the wrapper prose div.
         # So we need to inject the content INSIDE the prose div.
         
         # The regex should be:
         # (<div class="prose ...">) .*? (</div>\s*<!-- End Prose -->) -> IF we had comments.
         # Since we don't, we assume the block ends with the Run Date paragraph.
         ...
         # This matches Start -> Run Date line.
         # It does NOT match the content AFTER Run Date line (which should be nothing in the inner block).
         # So we replace \1 + new_content. 
         # The new_content INCLUDES the new Run Date line.
