...
def inject_to_html(filename, var_name, content, is_json=False):
    ...
    # --- SELF-HEALING: Fix broken Javascript functions if found ---
    if filename == "MoneyList2025.html":
        # Fix formatAmount function to be robust (handles numbers/nulls)
        old_func = r'function formatAmount\(amt\) \{.*?const num = parseFloat\(amt\.replace\(.*?\)\) \|\| 0;.*?\}'
        new_func = 'function formatAmount(amt) { if(!amt) return "$0.00"; const num = parseFloat(amt.toString().replace("$", "").replace(",", "")) || 0; return `$${num.toFixed(2)}`; }'
        html = re.sub(old_func, new_func, html, flags=re.DOTALL)

    # --- DATA INJECTION ---
    if is_json:
        # Use lowercase keys to match Javascript requirements
...
def run_pipeline():
    ...
    for df_to_merge, col_name in [(net_medal, 'net_medal_earnings'), (bb_df, 'BB_Earn'), (quota_df, 'Quota_Earn'), (gross_skins, 'Gskins_earnings'), (net_skins, 'Nskins_earnings')]:
    ...
    # Average Score (Fixing lowercase 'par' key)
    ...
    # Ensure keys are lowercase for JS
    hole_json = hole_avg[['hole', 'score', 'par']].to_dict('records')
    inject_to_html("AverageScore.html", "holeData", hole_json, is_json=True)

def sync():
    print("🚀 Pushing to GitHub...")
    subprocess.run(["git", "add", "."], cwd=BASE_PATH)
    subprocess.run(["git", "commit", "-m", "Master Site Fix: Corrected Par mapping and MoneyList logic"], cwd=BASE_PATH)
